/*
 * (C) Copyright ${year} Nuxeo SA (http://nuxeo.com/) and others.
 *
 * All rights reserved. This program and the accompanying materials
 * are made available under the terms of the GNU Lesser General Public License
 * (LGPL) version 2.1 which accompanies this distribution, and is available at
 * http://www.gnu.org/licenses/lgpl-2.1.html
 *
 * This library is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
 * Lesser General Public License for more details.
 */

'use strict';

angular.module('huddleAdmin')

    .controller('ViewEmailController', ['$scope', '$state', '$stateParams', 'EmailService',
        function ($scope, $state, $stateParams, EmailService) {

            var emailId = $stateParams.emailId;

            $scope.email = EmailService.get({id: emailId}, function(email) {
                // dodgy hack to update iframe content since angular doesn't seem to support this
                var iframeDocument = document.querySelector('#content-iframe').contentWindow.document;
                var content = email.content;
                iframeDocument.open('text/html', 'replace');
                iframeDocument.write(content);
                iframeDocument.close();
            });

        }]);
